...
jQuery(document).ready(function($){
	...
	if(window.location.pathname.length > 1){
		$('.search').val(unescape(window.location.pathname.substr(1)));
		checkFavoriteButton(window.location.pathname.substr(1));
	...
	$('#foo').submit(function(e){
		...
		window.history.pushState('', document.title, $('#foo .search').val().toLowerCase());

		//This stops the browser from actually following the link
		e.preventDefault();
		executeSearch(($('#foo .search').val()).toLowerCase());
		return false;
	});
	function checkFavoriteButton(word){
		...
		$.post(url,mydata, function(data){
				loadingImg = $('.mywl-img');
				if(data == 'favorite'){
					$('.mywl-link').css('color','red');
					loadingImg.hide();
				} else {
					$('.mywl-link').css('color','#d4d4d4');
					loadingImg.hide();
				}
		});
	}
		...
		if(m_result != null){
			...
			if(myResultsList.length == 1){
				...
				if(myres.match(/\[/) == null && myres.match(/\s/) == null ){
					window.history.pushState(myres.toLowerCase(), document.title, myres.toLowerCase());
					//myconsole(myres);
					executeSearch(myres);
				}
			}
	...
	$(window).bind('popstate', function(event) {
		if(!initialpop){
			if(window.location.pathname.length > 1){
				...
				executeSearch(currentpage.toLowerCase());
	...
	function rebindClick(){
		...
		$('div.a').click(function(event){
			...
			$('#result').animate({opacity: 0},100,function(){
				...
				window.history.pushState(linktext.toLowerCase(), document.title, linktext.toLowerCase());
				executeSearch(linktext.toLowerCase());
